...
public class StridedInterval implements Serializable {
	...
	protected StridedInterval(
			long stride, long lower, long upper, DataWidth dataWidth) {
		...
		if (stride < 0) {
			throw new IllegalArgumentException(
					"Invalid strided interval: stride must not be negative");
		} else if (stride == 0 && lower != upper) {
			throw new IllegalArgumentException(
					"Invalid strided interval: stride must not be zero");
		} else if ((lower > upper) && (stride != 1 || lower != 1
				|| upper != 0)) {
			throw new IllegalArgumentException(
					"Invalid strided interval: lower bound must not be larger than upper "
							+ "bound.");
		} else if (stride > 0 &&
				BigInteger.valueOf(upper)
				          .subtract(BigInteger.valueOf(lower))
				          .mod(BigInteger.valueOf(stride))
				          .longValue() != 0) {
			throw new IllegalArgumentException(
					"Invalid strided interval: upper bound must be tight.");
		}
	...
	public static StridedInterval getTop(DataWidth dataWidth) {
		switch (dataWidth.getWidth()) {
			...
			default:
				return getInterval(dataWidth.getMinimumValue(),
						dataWidth.getMaximumValue(), dataWidth);
		}
	}
	...
	public static StridedInterval getSingletonSet(
			long number, DataWidth dataWidth) {
		// singleton sets have a stride equal to 0
		return getStridedInterval(0, number, number, dataWidth);
	}

	public static StridedInterval getInterval(
			long lower, long upper, DataWidth dataWidth) {
		return getStridedInterval(1, lower, upper, dataWidth);
	}

	public static StridedInterval getStridedInterval(
			long stride, long lower, long upper, DataWidth dataWidth) {
		switch (dataWidth.getWidth()) {
			case 1:
	...
	public boolean contains(long number) {
		return !(lowerBound > number || upperBound < number) && (
				isSingletonSet() && lowerBound == number
	...
	public boolean contains(StridedInterval si) {
		if (isBottom()) {
			...
			return si.stride % this.stride == 0
					...
					&& si.lowerBound <= this.upperBound
					&& si.upperBound >= this.lowerBound
	...
	public StridedInterval add(StridedInterval si) {
		...
		long u = this.lowerBound & si.lowerBound & ~lower & ~(this.upperBound
				& si.upperBound & ~upper);
		long v = ((this.lowerBound ^ si.lowerBound) | ~(this.lowerBound
				^ lower)) & (~this.upperBound & ~si.upperBound
				& upper);
		...
		if ((u | v) < 0) {
			...
			return getStridedInterval(stride, dataWidth.effectiveValue(lower),
					dataWidth.effectiveValue(upper),
					dataWidth);
		}
	}
	...
	public StridedInterval union(StridedInterval si) {
		...
		} else if (this.lowerBound < si.lowerBound
				&& this.upperBound < si.upperBound) {
		...
		return getStridedInterval(stride, lowerBound, upperBound,
				DataWidth.maximum(dataWidth, si.dataWidth));
	}
	...
	public StridedInterval intersect(StridedInterval si) {
		...
		if (this.upperBound <= si.lowerBound) {
			...
			assert anchor == (this.upperBound - (j0
					* this.stride)) : "Anchor is invalid";

			long t = (Math.max(this.lowerBound, si.lowerBound) - anchor)
					/ answerStride;
			if (t >= 0 && ((Math.max(this.lowerBound, si.lowerBound) - anchor)
					% answerStride) != 0) {
				t++;
			}
			...
			if (answerLowerBound > Math.min(this.upperBound, si.upperBound)) {
				return getBottom(dataWidth);
			} else if (Math.min(this.upperBound, si.upperBound)
					- answerLowerBound < answerStride) {
				return getSingletonSet(answerLowerBound, dataWidth);
			} else {
				long answerSize = (Math.min(this.upperBound, si.upperBound)
						- answerLowerBound) / answerStride + 1;
				return getStridedInterval(answerStride, answerLowerBound,
						answerLowerBound + (answerSize - 1) * answerStride,
						dataWidth);
			}
		}
	}
	...
	public StridedInterval removeLowerBound() {
		if (lowerBound == dataWidth.getMinimumValue()) {
			...
			return getStridedInterval(1, dataWidth.getMinimumValue(),
					upperBound, dataWidth);
		} else {
			long lowerBound = dataWidth.getMinimumValue();
			long delta = BigInteger.valueOf(upperBound)
	...
	public StridedInterval removeUpperBound() {
		if (upperBound == dataWidth.getMaximumValue()) {
			...
			return getStridedInterval(1, lowerBound,
					dataWidth.getMaximumValue(), dataWidth);
		} else {
	...
	public StridedInterval negate() {
		if (isSingletonSet() && isLowerBoundMinimal()) {
			...
			return getStridedInterval(stride, -upperBound, -lowerBound,
					dataWidth);
		} else {
			return getTop(dataWidth);
		}
	...
	public StridedInterval or(StridedInterval si) {
		...
		long t = Long.min(Long.numberOfTrailingZeros(this.stride),
				Long.numberOfTrailingZeros(si.stride));
		long mask = (0x1L << t) - 1;

		long answerStride = 0x1L << t;
		long answerSharedSuffix =
				(this.lowerBound & mask) | (si.lowerBound & mask);

		// zero out suffix bits
		long answerLowerBound =
				minOrSigned((this.lowerBound & ~mask),
						(this.upperBound & ~mask), (si.lowerBound & ~mask), (
								si.upperBound & ~mask));
		long answerUpperBound =
				maxOrSigned((this.lowerBound & ~mask),
						(this.upperBound & ~mask), (si.lowerBound & ~mask), (
								si.upperBound & ~mask));
	...
	public StridedInterval widen(StridedInterval stridedInterval) {
		...
		answer = lowerBound
				<= stridedInterval.lowerBound ? answer : answer.removeLowerBound();
		answer = upperBound
				>= stridedInterval.upperBound ? answer : answer.removeUpperBound();
		return answer;
	}
	...
	public Iterable<Long> values() {
		return () -> new Iterator<Long>() {
			...
			private long nextValue = lowerBound;
			...
			@Override
			public Long next() {
				long answer = nextValue;
				nextValue += (stride > 0 ? stride : 1);
				return answer;
			}
		};
	}
