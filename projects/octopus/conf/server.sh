#!/bin/sh
# Replacement orientdb-*/bin/server.sh for octopus.
...
exec "$JAVA" $JAVA_OPTS $MAXHEAP $JAVA_OPTS_SCRIPT $ORIENTDB_SETTINGS $MAXDISKCACHE -Djava.util.logging.config.file="$LOG_FILE" -Dorientdb.config.file="$CONFIG_FILE" -Dorientdb.www.path="$WWW_PATH" -Dorientdb.build.number="2.1.x@r${buildNumber}; 2015-10-29 16:54:25+0000" -cp "$ORIENTDB_HOME/lib/orientdb-server-2.1.5.jar:$ORIENTDB_HOME/lib/*:$ORIENTDB_HOME/../jars/*" $* com.orientechnologies.orient.server.OServerMain
